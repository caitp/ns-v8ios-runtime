...
void Interop::WriteValue(Local<Context> context, const TypeEncoding* typeEncoding, void* dest, Local<Value> arg) {
    ...
    if (arg.IsEmpty() || arg->IsNullOrUndefined()) {
        ...
        if (baseWrapper != nullptr && baseWrapper->Type() == WrapperType::Block) {
            ...
            BlockWrapper* wrapper = MakeGarbageCollected<BlockWrapper>(isolate, (void*)blockPtr, blockTypeEncoding);
...
Local<Value> Interop::GetResult(Local<Context> context, const TypeEncoding* typeEncoding, BaseCall* call, bool marshalToPrimitive, Local<Value> parentStruct, bool isStructMember, bool ownsReturnedObject, bool returnsUnmanaged, bool isInitializer) {
    ...
    if (typeEncoding->type == BinaryTypeEncodingType::BlockEncoding) {
        ...
        BlockWrapper* blockWrapper = MakeGarbageCollected<BlockWrapper>(isolate, block, typeEncoding);
